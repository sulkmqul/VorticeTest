...
namespace SerialVortice
{
    ...
    /// </summary>
    internal class VorticeTest : IDisposable
    {
        public VorticeTest()
        {
        }
        ...
        public void Init()
        {
            ...
            //Textureの読み込み
            this.TextureResouce = this.ReadTexture(@"image\testimage.png");

            //Shaderの初期化
            this.InitShader(@"shader\shader.fx");
        }
        ...
        public void Loop()
        {
            ...
            ClarityLoop.Run(this.Form, () =>
            {
                ...
                //位置更新
                Matrix4x4 tm = Matrix4x4.CreateTranslation(new Vector3(0.0f, 0.0f, 0.0f));
                Matrix4x4 rm = Matrix4x4.CreateRotationY(rot);
                Matrix4x4 sm = Matrix4x4.CreateScale(500.0f);
        ...
        private ID3D11ShaderResourceView ReadTexture(string filepath)
        {
            ID3D11ShaderResourceView ans = null;
            try
            {
                using (Bitmap srcbit = new Bitmap(filepath))
                {
        ...
        private void InitShader(string shaderfilepath)
        {
            //VertexShaderの作成
            this.CreateVertexShader(shaderfilepath);

            //PixelShader作成
            this.CreatePixelShader(shaderfilepath);
        ...
        private void CreateVertexShader(string filepath)
        {
            try
            {
                //Shaderファイルの読み込み
                string srccode = System.IO.File.ReadAllText(filepath);
                ...
                {
                    throw new Exception($"Vs Compile FAILED ret={cpret}");
                }
        ...
        /// </summary>
        /// <param name="filepath">shaderファイルパス</param>
        protected void CreatePixelShader(string filepath)
        {
            try
            {
                //Shaderファイルの読み込み
